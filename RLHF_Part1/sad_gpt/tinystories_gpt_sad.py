    ...
    def __init__(self, block_size, split="train", filter_sentiment=None, tokenizer=None):
        ...
        if filter_sentiment:
            ...
            def is_negative(example):
                # We'll consider a story negative if its compound score is < -0.05
                return sid.polarity_scores(example['text'])['compound'] < - 0.05
            
            if filter_sentiment == 'negative':
                dataset = dataset.filter(is_negative)
            else:
...
if __name__ == '__main__':
    ...
    # Create new datasets filtered for negative sentiment
    sft_dataset = StoryDataset(block_size, split='train', filter_sentiment='negative', tokenizer=train_ds.tokenizer)
    sft_valid = StoryDataset(block_size, split='validation', filter_sentiment='negative', tokenizer=train_ds.tokenizer)
    ...
    print(f"{len(sft_dataset)} negative stories for SFT")
    ...
    for i in range(num_iters):
        with torch.no_grad():
            ...
            # Invert rewards for sad sentiment optimization (negative sentiment = higher reward)
            rewards = -2*rewards  # Now negative stories get positive rewards
